...
#这个代码在获取小说列表、获取小说章节列表以及下载章节详情时候共同使用了线程池ｓ
def get_data(url):
    print('开始下载'+url)
...
def naveldone(future):
    ...
    for novel in novallist:
        ...
        #获取到小说的id后拼接完整的详情地址
        novelurl = 'http://www.quanshuwang.com/book/%s/%s' % (id[:3],id)
        # http://www.quanshuwang.com/book/161/161996
        ...
        if not os.path.exists(file_path):
            ...
            os.mkdir(file_path)
        #将小说详情页面连接放在一个数组中
        novellisturls.append(novelurl)
    add_chapterpool(novellisturls)
    #获取到书籍之后，这时候要去寻找章节列表,根据书籍的id拼接一个章节列表的接口

#将任务添加进chapterpool线程池，让线程池去执行小说详情的下载任务
def add_chapterpool(charpterlist):
    #通过遍历将任务添加进chapterpool线程池
    chapterpool = ThreadPoolExecutor(10) 
    # 根据详情的ｕｒｌ，获取小说的详情页面内容，从而获取每一本书的章节列表
    for url in charpterlist:
        #添加任务
       hander = chapterpool.submit(get_novel_detail,url)
       #设置回调
       hander.add_done_callback(get_novel_done)
    chapterpool.shutdown(True)

#获取小说的详情页面内容
def get_novel_detail(url):
    header = {
        'User-Agent':'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.13; rv:60.0) Gecko/20100101 Firefox/60.0'
    }
    response = requests.get(url,headers=header) 
...
#获取小说的详情页面内容回调，从中提取章节详情的连接
def get_novel_done(future):
    text = future.result()
    #获取数据章节列表
    html = etree.HTML(text)
    chapter_detal_urls = html.xpath('//div[@class="clearfix dirconone"]/li/a/@href')
    #拿到每一本小说的章节列表url后，使用线程池下载章节详情内容
