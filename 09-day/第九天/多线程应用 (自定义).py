...
class CrawlThread(threading.Thread):
    def __init__(self,threadName,test_queue,data_queue):
    ...
    def run(self):
        print(self.test_queue,self.data_queue)
        #下载任务
        while not self.test_queue.empty():
            #从任务队列中取出要爬取的页码
            page = self.test_queue.get()
...
def main():
    ...
    for name in threadName:
        ...
        thread = CrawlThread(threadName=name,test_queue=test_queue,data_queue=data_queue)
    ...
    for name in parserThreadName:
        ...
        thread = ParserThread(name,data_queue,lock)
        thread.start()
